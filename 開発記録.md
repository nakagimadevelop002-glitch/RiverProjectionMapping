...
### Phase 22（続き）: MeasurementLogger UI Text更新問題の修正

#### 問題
- 計測結果ログがUI Textに表示されない
- 計測成功・失敗時のログが反映されない

#### 原因調査
MeasurementLogger.csのUpdateText()実装を確認：
- `string.Join("\n", logQueue)` - Queue<string>を直接Joinしていた
- logTextがnullの場合のエラーハンドリングなし
- デバッグログがなく、動作確認困難

#### 修正内容（MeasurementLogger.cs）

**1. UpdateText()の改善（99-121行）**:
```csharp
...
**2. デバッグログ追加**:
- AddLog()（96行）: `[MeasurementLogger] ログ追加: {log}`
- Update()（34行）: `[MeasurementLogger] メインスレッドでログ処理: {log}, logQueue.Count={logQueue.Count}`
- UpdateText()（119行）: `[MeasurementLogger] logTextが設定されていません`（警告）

#### 効果
- ✅ Queue<string>を配列に変換してから確実にJoin
- ✅ logTextがnullの場合は警告ログで即座に検知可能
- ✅ ログ追加・処理の各段階をデバッグログで追跡可能
- ✅ 計測成功・失敗時のログが確実にUI Textに反映

#### 追加デバッグログ（メソッド呼び出し確認用）
- LogSuccess()（53行）: `[MeasurementLogger] LogSuccess呼び出し: {speed}`
- LogFailure()（64行）: `[MeasurementLogger] LogFailure呼び出し: speed={speed}, threshold={threshold}`
- LogStart()（75行）: `[MeasurementLogger] LogStart呼び出し`
- LogError()（86行）: `[MeasurementLogger] LogError呼び出し: {message}`

#### CameraSpeedReceiver.cs修正
- logger null チェック強化（212-220行）
- logger.LogStart()呼び出し前にデバッグログ出力

#### 確認事項（ユーザー側）
- [x] MeasurementLoggerコンポーネントのInspectorで`Log Text`フィールドにUI Text設定
- [x] CameraSpeedReceiverコンポーネントのInspectorで`Logger`フィールドにMeasurementLogger設定
- [x] デバッグログで動作確認完了
- [x] 計測失敗時のログがUI Textに表示されることを確認

#### 成果
- ✅ 計測開始・成功・失敗・エラーの全ログがUI Textに正常表示
- ✅ Thread-safe処理で別スレッドからのログ追加に対応
- ✅ デバッグログで各処理段階を追跡可能
- ✅ Inspector設定ミスを即座に検知可能（警告ログ）

---

## 📅 2026-01-25（続き2）

### Phase 23: 速度閾値判定の修正（計測成功ログ表示問題の解決）

#### 問題
- 計測成功時にUI Textにログが表示されない
- 計測開始・失敗ログは表示されるが、成功ログが出ない
- コンソールログ: `-0.004069 m/s < 0.01 m/s` → 失敗判定
- コンソールログ: `0.004605 m/s < 0.01 m/s` → 失敗判定

#### 根本原因
**速度判定ロジックの誤り**（CameraSpeedReceiver.cs:306）:
```csharp
if (speed < minimumValidSpeed)  // 負の値は常にこの条件に該当
```

**問題点**:
- 負の速度（逆流）が常に閾値以下と判定される
- STIV法では負の速度も有効な計測結果（Phase 21参照）
  - 正の速度：左から右の流れ
  - 負の速度：右から左の流れ（逆流）

#### 修正内容（CameraSpeedReceiver.cs:306-309）
...
#### 効果
- ✅ 負の速度も有効な計測結果として認識
- ✅ 速度の絶対値が閾値以上なら計測成功と判定
- ✅ UI Textに計測成功ログが正常表示
- ✅ 逆流（負の速度）も正しく計測・表示可能

#### 判定例
- `|−0.004069| = 0.004069 < 0.01` → 失敗（閾値以下）
- `|0.004605| = 0.004605 < 0.01` → 失敗（閾値以下）
- `|−0.015| = 0.015 > 0.01` → 成功（逆流を検出）
- `|0.015| = 0.015 > 0.01` → 成功（順流を検出）

---

## 📅 2026-01-25（続き3）

### Phase 24: 速度単位の修正（実世界速度と正規化座標系の変換）

#### 問題
- Pythonから計測した速度を適用すると、粒子が実際の川よりはるかに速く動く
- 原因：速度の単位が混在
  - `velocity`：正規化座標系（0-1）での速度
  - `speedMultiplier`：実世界の速度（m/s）
  - これらを直接掛け算していた

#### 根本原因（RiverFlowSimulation.cs:318）
```csharp
p.position += velocity * dt * speedMultiplier * ySpeedMultiplier;
```

**問題点**:
- VectorField.baseSpeed = 0.5（正規化座標系）
- speedMultiplier = 0.004605（m/s）
- 単位が異なる値を掛け算 → 意味不明な結果

#### 修正内容

**1. 基準速度パラメータ追加**（RiverFlowSimulation.cs:34-36）:
```csharp
...
**2. SetSpeedMultiplier()修正**（RiverFlowSimulation.cs:444-462）:
```csharp
public void SetSpeedMultiplier(float realWorldSpeed)
{
    ...
    if (mode == SimulationMode.NormalMode)
    {
        ...
        UnityEngine.Debug.Log($"[RiverFlowSimulation] 速度設定: {realWorldSpeed:F6} m/s → speedMultiplier = {ratio:F3} (基準速度: {baseRealWorldSpeed} m/s)");
    }
    else if (mode == SimulationMode.WaveMode)
    {
        waveSpeed = ratio;
        UnityEngine.Debug.Log($"[RiverFlowSimulation] 速度設定: {realWorldSpeed:F6} m/s → waveSpeed = {ratio:F3} (基準速度: {baseRealWorldSpeed} m/s)");
    }
}
```
...
**基準速度方式**:
- `baseRealWorldSpeed = 0.5 m/s`（デフォルト）の時、speedMultiplier = 1.0となる
- Pythonから受信した速度を基準速度で割って比率に変換
- 比率をspeedMultiplier/waveSpeedに設定

**計算例**:
- Pythonから受信: 0.015 m/s
- 基準速度: 0.5 m/s
- 比率: 0.015 / 0.5 = 0.03
- speedMultiplier = 0.03（元の速度の3%）

#### 効果
- ✅ 実世界の速度（m/s）が正しく正規化座標系の速度に変換される
- ✅ 粒子の速度が実際の川の速度と一致する
- ✅ 基準速度をInspector上で調整可能
- ✅ デバッグログで変換後の値を確認可能

#### 調整方法
**基準速度が適切でない場合**:
1. Inspector上で`Base Real World Speed`を調整
2. 実際の川の速度計測結果に基づいて設定
3. speedMultiplier = 1.0の時の粒子速度が適切になるように調整

---

## 📅 2026-01-25（続き4）

### Phase 25: 速度調整係数の実装（視覚的速度補正機能）

#### 背景・目的
- 粒子は投影するが、川を計測・観測する位置は川から離れている
- 実際の速度より遅く見える問題が残存
- 解決策：受信速度に掛ける調整係数をUI上で手動調整可能にする

#### 実装内容

**1. 速度調整係数パラメータ追加**（RiverFlowSimulation.cs:38-40）:
```csharp
...
**2. SetSpeedMultiplier()修正**（RiverFlowSimulation.cs:448-463）:
```csharp
public void SetSpeedMultiplier(float realWorldSpeed)
{
    // 基準速度に対する比率を計算 × 調整係数
...
**3. UI制御関数追加**（RiverFlowSimulation.cs:494-502）:
```csharp
/// <summary>
...
**速度変換の全体像**:
```
1. Pythonから受信: realWorldSpeed（例: 0.015 m/s）
2. 基準速度比率: 0.015 / 0.5 = 0.03
3. 調整係数適用: 0.03 × speedAdjustmentFactor
4. 最終速度: speedMultiplier/waveSpeed
```

**計算例**（speedAdjustmentFactor = 3.0の場合）:
```
受信速度: 0.015 m/s
基準速度: 0.5 m/s
調整係数: 3.0
→ (0.015 / 0.5) × 3.0 = 0.09
→ speedMultiplier = 0.09（元の速度の9%）
```
...
```
Slider → OnValueChanged(Single)
  └─ Dynamic float → SetSpeedAdjustmentFactor
```
...
- ✅ 観測位置・角度による見え方の違いを補正可能
- ✅ 実世界の速度を正確に計測しながら、視覚的な調整が可能
- ✅ UI上でリアルタイムに調整可能
- ✅ デバッグログで調整後の値を確認可能
...
**粒子が遅すぎる場合**:
- speedAdjustmentFactorを2.0, 3.0と増やす
- 肉眼で見た川の速度と一致するまで調整

**粒子が速すぎる場合**:
- speedAdjustmentFactorを0.5, 0.3と減らす

**最適な調整方法**:
1. まず基準速度（baseRealWorldSpeed）を実際の川の平均速度に合わせる
2. 次に調整係数（speedAdjustmentFactor）で視覚的な微調整を行う

---

## 📅 2026-01-27

### Phase 26: 計測機能ON/OFF制御（完了 ✅）

#### 目的
ビルド後のランタイムUIで計測機能全体をON/OFFできるトグルを実装

#### 実装内容

**1. UI制御関数追加**（CameraSpeedReceiver.cs:210-239）:
```csharp
/// <summary>
...
**2. UI設定方法**:
- Toggle → OnValueChanged(Boolean) → Dynamic bool → SetMeasurementDisabled
- Toggle.isOn = false（デフォルト）: 計測有効
- Toggle.isOn = true（チェック時）: 計測停止

#### 動作
- **デフォルト（チェックなし）**: 計測機能ON（measurementEnabled = true）
- **チェック時**: 計測機能OFF（measurementEnabled = false）
  - 定期計測停止（CancelInvoke）
  - 即時計測ボタン無効化
  - 実行中のPythonプロセスを即座に停止
- **チェック解除時**: 計測機能ON復帰
  - 定期計測再開（InvokeRepeating）

#### 効果
- ✅ ビルド後のランタイムで計測機能全体をON/OFF可能
- ✅ チェック時に実行中の計測も即座に停止
- ✅ チェック解除時に定期計測が自動再開
- ✅ デバッグログで状態変化を確認可能
- ✅ ユーザーの要求通りの動作（デフォルトON、チェックでOFF）

#### 修正履歴
**初回実装の問題**: 有効化時（チェック解除時）にInvokeRepeatingの再開処理が欠落
- 無効化後、再度有効化してもインターバル計測が開始されない不具合
- 修正: else節に`InvokeRepeating(nameof(StartMeasurement), 0f, measurementInterval)`を追加

---

## 📅 2026-01-27（続き）

### Phase 27: カメラ名UI入力対応（完了 ✅）

#### 目的
ビルド後のランタイムUIでカメラ名を入力し、外部カメラを使用可能にする

#### 背景
- 現在：外部カメラはビルド前にInspector設定が必要
- 要求：UI InputFieldでカメラ名を入力すれば使えるようにする

#### 実装内容

**1. UI制御関数追加**（CameraSpeedReceiver.cs:199-208）:
```csharp
/// <summary>
...
**2. InputFieldフィールド追加**（CameraSpeedReceiver.cs:70-71）:
```csharp
[Tooltip("カメラ名入力フィールド（初期値をInspector設定値に同期）")]
public UnityEngine.UI.InputField cameraNameInputField;
```

**3. Start()での初期値同期**（CameraSpeedReceiver.cs:104-108）:
```csharp
...
**4. UI設定方法**:
- InputField → OnValueChanged(String) → Dynamic string → SetCameraName
- InspectorでCamera Name Input Fieldフィールドに設定
- 起動時、Inspector設定値がInputFieldの初期値になる

#### 動作
- **cameraId = 0, 1**: カメラIDで直接指定（従来通り）
- **cameraId = 2以降**: cameraNameで指定
  - baseline_stiv.pyの`find_camera_id_by_name()`でカメラ名→ID変換
  - WMIでWindowsカメラリストを検索
  - 一致するカメラのOpenCV IDを特定

#### 使用例

**ビルド前の準備**:
1. InspectorでcameraNameに`HD Webcam eMeet C960`を設定
2. InspectorでCamera Name Input FieldにInputFieldを設定

**ビルド後の動作**:
1. 起動時、InputFieldに`HD Webcam eMeet C960`が表示される（Inspector設定値）
2. UI Sliderで`cameraId = 2`に設定
3. 計測開始 → eMeet C960が使用される
4. InputFieldを編集すれば、別のカメラに変更可能

#### 効果
- ✅ ビルド後のランタイムで外部カメラ名を入力可能
- ✅ 複数の外部カメラを切り替え可能
- ✅ Inspector設定値がInputFieldの初期値になる
- ✅ デバッグログでカメラ名設定を確認可能
- ✅ ビルド後もカメラ名を自由に変更可能
...
### Phase 28: カメラエラー時の例外処理強化（完了 ✅）

#### 問題
カメラ名が無効な場合、Pythonから`[ERROR] Failed to find camera by name:`が出力されるが、UI Text（logger）に記録されない

#### 原因
OnErrorDataReceivedの条件が不十分（CameraSpeedReceiver.cs:420）:
```csharp
// 旧: OSError、IOError、Exceptionで始まる行のみ記録
if (logger != null && (e.Data.StartsWith("OSError") || e.Data.StartsWith("IOError") || e.Data.StartsWith("Exception")))
```

カメラエラーは`[ERROR]`で始まるため、条件に該当せず記録されない。

#### 修正内容（CameraSpeedReceiver.cs:420）
...
**追加した条件**:
- `e.Data.StartsWith("[ERROR]")` - Pythonの標準エラーフォーマット
- `e.Data.Contains("Camera")` - カメラ関連エラー全般

#### 既存の例外処理（確認済み）

**1. C#側のtry-catch**（346-350行）:
- Pythonプロセス起動失敗をキャッチ
- `isProcessing = false` でクリーンアップ
- アプリは停止しない

**2. カメラ名空チェック**（305-310行）:
- カメラ名が空の場合、警告ログ出力
- `isProcessing = false` でクリーンアップ
- アプリは停止しない

**3. OnProcessExited**（430-445行）:
- Pythonプロセス終了時に必ず`isProcessing = false`
- イベントハンドラ解除
- アプリは停止しない

#### 効果
- ✅ カメラ名が無効でもアプリは停止しない
- ✅ Pythonエラーは全てUnityコンソールに出力
- ✅ カメラ関連エラーはUI Text（logger）にも表示
- ✅ `[ERROR]`形式のエラーメッセージも記録
- ✅ プロセス終了時に必ずクリーンアップ
- ✅ 次回の計測実行に影響しない
